...
void SpMV(struct CSR MatrCSR, double* x, double* ans) {	//ans = Ax
	for (int i = 0; i < nDim; i++){
			double sum = 0.0;
...
void Axpy(double* x, double* y, double a, int flag, int nDim) { 	// 1: x=x+ay   2: y=x+ay
	if (!(flag==1||flag==2)) {
...
void Vcopy(double* x, double* y, int nDim) { 	// y = x 
	for (int i = 0; i < nDim; i++) y[i] = x[i];	
}

void Vdiff(double*x, double* y, double* z, int nDim){	// z = x - y
...
double* Slove_SOLE_sequential(struct CSR MatrCSR,double* VecB, double init_val, int nDim){
	...
	do{
		...
		//printf("%d: %lf %lf\n",tot_iteration,tau,Norm_Vec(tmp, nDim));

	} while (Norm_Vec(tmp, nDim) > eps);
	//} while (Norm_Vec(VecR) > eps);
...
int main(int argc,char** argv) {
	...
	VecRes = Slove_SOLE_sequential(MatrCSR,VecB,initial_value,nDim);
